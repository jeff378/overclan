...
export default function ReplayPage() {
  ...
  const loadComments = async (postId: string) => {
    const { data } = await supabase.from("replay_comments").select("*").eq("post_id", postId).order("created_at", { ascending: true });
    if (data) {
      const withProfiles = await fetchWithProfiles(data);
  ...
  const handleVote = async (post: any, vote: string) => {
    if (!user || myVotes[post.id]) return;
    ...
    <div style={{ minHeight: "100vh", background: "#080c14", color: "#e8eaf0", fontFamily: "'Rajdhani', 'Noto Sans KR', sans-serif" }}>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .vote-btn { padding: 10px 20px; font-family: 'Rajdhani', sans-serif; font-size: 13px; font-weight: 700; letter-spacing: 1px; cursor: pointer; clip-path: polygon(8px 0%, 100% 0%, calc(100% - 8px) 100%, 0% 100%); transition: all 0.2s; border: none; }
        .vote-hack { background: rgba(239,83,80,0.2); color: #ef5350; border: 1px solid rgba(239,83,80,0.4) !important; }
        .vote-hack.voted, .vote-hack:hover { background: rgba(239,83,80,0.4); }
        .vote-clean { background: rgba(76,175,80,0.2); color: #4caf50; border: 1px solid rgba(76,175,80,0.4) !important; }
        .vote-clean.voted, .vote-clean:hover { background: rgba(76,175,80,0.4); }
            ...
            <div style={{ background: "rgba(13,20,35,0.8)", border: "1px solid rgba(255,107,35,0.15)", padding: "28px" }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                  <button className={`vote-btn vote-hack ${myVotes[selected.id] === "핵" ? "voted" : ""}`} onClick={() => handleVote(selected, "핵")} disabled={!!myVotes[selected.id]}>🚨 핵 맞음 ({selected.votes_hack || 0})</button>
                  <button className={`vote-btn vote-clean ${myVotes[selected.id] === "정상" ? "voted" : ""}`} onClick={() => handleVote(selected, "정상")} disabled={!!myVotes[selected.id]}>✅ 정상 ({selected.votes_clean || 0})</button>
                </div>
                {getVotePercent(selected).total > 0 && (
                  <div>
