...
// 간이 레이트리밋 (워밍된 인스턴스당 메모리 — curl 폭격 1차 차단. 영구 보장은 Vercel KV/Upstash로 후속)
const RL_MAX = 15; // 윈도우당 허용 요청
const RL_WINDOW = 10 * 60 * 1000; // 10분
const hits = new Map<string, { count: number; resetAt: number }>();
function rateLimited(ip: string): boolean {
  const now = Date.now();
  const e = hits.get(ip);
  if (!e || now > e.resetAt) { hits.set(ip, { count: 1, resetAt: now + RL_WINDOW }); return false; }
  e.count++;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (rateLimited(ip)) {
      return NextResponse.json({ text: "문의가 너무 많아요. 잠시 후 다시 시도해주세요.", escalate: false }, { status: 429 });
    }

    const { messages } = await req.json();

    // 입력 검증 — 과금 어뷰징 방지 (배열/개수/길이 제한)
    if (!Array.isArray(messages) || messages.length === 0 || messages.length > 30) {
      return NextResponse.json({ text: "잘못된 요청이에요.", escalate: false }, { status: 400 });
    }
